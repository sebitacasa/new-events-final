...
const getAllEvent = async () => {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // ✅ Verificamos si ya tiene tickets antes de crearlos
      const existingTickets = await Ticket.count({
        where: { EventId: eventDB.id },
      });

      if (existingTickets === 0) {
        const ticketArray = [];

        for (let j = 0; j < eventDB.stock; j++) {
          ticketArray.push({
            status: "Disponible",
            EventId: eventDB.id, // <- importante
          });
        }

        await Ticket.bulkCreate(ticketArray);
      }
    }
  } catch (error) {
    console.log(error);
